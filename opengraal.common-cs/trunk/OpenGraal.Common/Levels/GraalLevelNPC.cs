...
namespace OpenGraal.Common.Levels
{
	public class GraalLevelNPC : IRefObject, IGaniObj
	{
		...
		public void SendProp (Properties PropId)
		{
			...
			this.Server.SendPacket (new CString () + (byte)3 + (int)this.Id + (byte)PropId + GetProp (PropId));
		}
		...
		public void SetProps (CString Packet)
		{
			while (Packet.BytesLeft > 0)
			{
				...
				case Properties.SCRIPT: // 1
					...
					this.Script = Packet.readChars (Packet.readGUShort ()).Text;
				...
				case Properties.NPCX: // 2 - obsolete
					//Packet.ReadGByte1 ();
					this.OldX = (float)(Packet.ReadGByte1 ()) / 2.0f;
					this.PixelX = (int)(this.OldX * 16);
					break;

				case Properties.NPCY: // 3 - obsolete
					//Packet.ReadGByte1 ();
					this.OldY = (float)(Packet.ReadGByte1 ()) / 2.0f;
				...
				case Properties.NPCID: // 17
					this.Id = (int)Packet.ReadGUByte3 ();
					break;

				case Properties.SPRITE: // 18
					this.Dir = Packet.ReadGUByte1 ();
					break;
				...
				default:
					System.Console.WriteLine ("Unknown NPC Prop: " + PropId + " Data: " + Packet.Text);
					Packet.ReadGUByte1 ();
					return;
				}
			}
