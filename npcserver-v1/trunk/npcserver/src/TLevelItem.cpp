...
CString TLevelItem::getItemPlayerProp(const CString& pItemName, TPlayer* player)
{
	...
	switch (itemID)
	{
		...
		case 3:		// bombs
		{
			CString playerProp = player->GetPlayerProp(PLPROP_BOMBSCOUNT);
		...
		case 4:		// darts
		{
			CString playerProp = player->GetPlayerProp(PLPROP_ARROWSCOUNT);
		...
		case 5:		// heart
		{
			CString playerProp = player->GetPlayerProp(PLPROP_CURPOWER);
			char heartCount = playerProp.readGChar() + (1 * 2);
			playerProp = player->GetPlayerProp(PLPROP_MAXPOWER);
		...
		case 11:	// fullheart
		{
			CString playerProp = player->GetPlayerProp(PLPROP_MAXPOWER);
		...
		case 24:	// spinattack
		{
			CString playerProp = player->GetPlayerProp(PLPROP_STATUS);
