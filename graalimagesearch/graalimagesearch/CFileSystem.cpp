...
void CFileSystem::addDir(const CString& dir, const CString& wildcard)
{
	...
	CString ndir = CString() << getHomePath() << newDir << wildcard;
...
void CFileSystem::addFile(CString file)
{
	...
	if (directory.find(getHomePath()) != -1)
		directory.removeI(0, getHomePath().length());

	// Add to the map.
	fileList[filename] = CString() << getHomePath() << directory << filename;
}
		...
		do
		{
			if (filedata.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				if (filedata.cFileName[0] != '.' && recursive)
				{
					...
					newDir.removeI(0, getHomePath().length());
					addDir(newDir);
				}
			}
			else
