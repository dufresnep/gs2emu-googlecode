...
enum {
    ...
    CANRESETATTRIBS = 256,
    CANADMINMSG = 512,
    CANSETSERVERFLAG = 1024,
    CANCHANGERIGHTS = 2048,
    CANBAN = 4096,
    CANCHANGECOMMENTS = 8192,
    CANINVISONLIST = 16384,
    CANCHANGESTAFFACC = 32768,
    CANSETSERVEROPTION = 65536,
    ...
    public CAccount
{
    ...
    int key, iterator, id, type, udpPort, lastMessage;
    // counters
    int lastData, lastMovement, lastNick, lastSave;
    #ifdef GSERV22
    codec mcodec;
    #endif
    CString version, guildName, language;
    CString ganiAttribs[5];
    CStringList weaponSend;
    //Player stats
    int additionalFlags, statusMsg, packCount, bombPower, carrySprite;
	int rating, failAttempts;
    CString gAni, bowImage, chatMsg;
    CLevel* level;
	...
	void warp(CString& pLevel, float pX, float pY);
	void leaveLevel();
...
{
    return (((adminRights & pRight) > 0 || adminLevel >= 5) && adminLevel > 0);
}
