...
namespace CS_NPCServer
{
	public class GameCompiler
	{
		...
		/// </summary>
		internal GameCompiler(NPCServer Server)
		{
			this.Server = Server;
		...
		public Assembly CompileScript(IRefObject ScriptObject, out CompilerResults Result)
		{
			...
			Result = csProvider.CompileAssemblyFromSource(options, "using CS_NPCServer; public class " + PrefixName + NextId[(int)ScriptObject.Type] + " : " + ClassName + " { public " + PrefixName + NextId[(int)ScriptObject.Type] + "(NPCServer Server, IRefObject Ref) : base(Server, Ref) { } " + Script + "\n } ");
		...
		public ScriptObj RunScript(IRefObject Reference, Assembly Script)
		{
			...
			foreach (Type type in types)
			{
				...
				ConstructorInfo constructor = type.GetConstructor(new Type[] { typeof(NPCServer), typeof(IRefObject) });
