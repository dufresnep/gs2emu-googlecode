...
namespace CS_NPCServer
{
	public class GraalPlayer
	{
		...
		protected DataBuffer PropBuffer = new DataBuffer();
		#endregion
		...
		/// </summary>
		internal GraalPlayer(NPCServer Server, Int16 Id)
		{
			this.Server = Server;
			this.Id = Id;
		...
		{
			// fix packet for flag
			Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLSETFLAG + (short)Id + FlagName + "=" + FlagValue);
		}
		...
		/// Get Property
		/// </summary>
		internal DataBuffer GetProp(Properties PropId)
		{
			switch (PropId)
			{
				case Properties.NICKNAME: // 0
					return new DataBuffer() + (byte)this.Nickname.Length + this.Nickname;

				case Properties.MAXPOWER: // 1
					return new DataBuffer() + (byte)this.FullHearts;

				case Properties.CURPOWER: // 2
					return new DataBuffer() + (byte)(this.Hearts * 2);

				case Properties.RUPEESCOUNT: // 3
					return new DataBuffer().WriteGByte3(this.Gralats);

				case Properties.ARROWSCOUNT: // 4
					return new DataBuffer() + (byte)this.Arrows;

				case Properties.BOMBSCOUNT: // 5
					return new DataBuffer() + (byte)this.Bombs;

				case Properties.GLOVEPOWER: // 6
					return new DataBuffer() + (byte)this.GlovePower;

				case Properties.BOMBPOWER: // 7
					return new DataBuffer() + (byte)this.BombPower;

				case Properties.SWORDPOWER: // 8
					return new DataBuffer() + (byte)(this.SwordPower + 30) + (byte)this.SwordImage.Length + this.SwordImage;

				case Properties.SHIELDPOWER: // 9
					return new DataBuffer() + (byte)(this.ShieldPower + 10) + (byte)this.ShieldImage.Length + this.ShieldImage;

				case Properties.ANIMATION: // 10
					return new DataBuffer() + (byte)this.Ani.Length + this.Ani;

				case Properties.HEADIMAGE: // 11
					return new DataBuffer() + (byte)(this.HeadImage.Length+100) + this.HeadImage;

				case Properties.CURCHAT: // 12
					return new DataBuffer() + (byte)this.Chat.Length + this.Chat;

				case Properties.PLCOLORS: // 13
					return ColorManager.GetPacket();

				case Properties.PLAYERX: // 15
					return new DataBuffer() + (byte)(this.PixelX / 8);

				case Properties.PLAYERY: // 16
					return new DataBuffer() + (byte)(this.PixelY / 8);

				case Properties.PLSPRITE: // 17
					return new DataBuffer() + (byte)(this.Dir % 4);

				case Properties.PLSTATUS: // 8
					return new DataBuffer() + (byte)this.PlayerStatus;

				case Properties.KILLSCOUNT: // 27
					return new DataBuffer() + (int)this.Kills;

				case Properties.DEATHSCOUNT: // 28
					return new DataBuffer() + (int)this.Deaths;

				case Properties.ONLINESECS: // 29
					return new DataBuffer() + (int)this.OnlineTime;

				case Properties.ALIGNMENT: // 32
					return new DataBuffer() + (byte)this.Ap;

				case Properties.BODYIMAGE: // 35
					return new DataBuffer() + (byte)this.BodyImage.Length + this.BodyImage;
				...
				case Properties.PIXELX: // 75
				{
					...
					if (PixelX < 0)
						res |= 0x0001;
					return new DataBuffer() + (short)res;
				}
		...
		/// Set Properties
		/// </summary>
		internal void SetProps(DataBuffer Packet)
		{
			bool moved = false;
		...
		{
			PropBuffer.Write(new DataBuffer() + (byte)PropId + GetProp(PropId));
			//	Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLSETPROPS + (short)this.Id + (byte)PropId + GetProp(PropId));
		}

		/// <summary>
		/// Send Property Changes
		/// </summary>
		internal void SendPropBuffer()
		{
			if (PropBuffer.Length > 0)
			{
				Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLSETPROPS + (short)this.Id + PropBuffer);
				PropBuffer.Clear();
			}
		}
		#endregion
		...
		public void AddWeapon(ServerWeapon Weapon)
		{
			...
			Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLADDWEP + (short)Id + Weapon.Name);
		...
		public void DeleteWeapon(String WeaponName, bool SendToServer)
		{
			...
			{
				if (SendToServer)
					Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLDELWEP + (short)Id + WeaponName);
		...
		public void DisableWeapons()
		{
			if ((this.PlayerStatus & (int)Status.ALLOWWEAPONS) != 0)
			{
				...
				Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLSETSTATUS + (short)id + (byte)1 + (byte)Status.ALLOWWEAPONS);
			}
		}
		...
		public void EnableWeapons()
		{
			...
			{
				this.PlayerStatus |= (int)Status.ALLOWWEAPONS;
				Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLSETSTATUS + (short)id + (byte)0 + (byte)Status.ALLOWWEAPONS);
			}
		}
		#endregion
		...
		{
			Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLMSGSIGN + (short)Id + Message);
		}
		...
		{
			Server.SendGSPacket(new DataBuffer() + (byte)GServerConnection.PacketOut.NCQUERY + (byte)GServerConnection.NCREQ.PLRPGMSG + (short)Id + DataBuffer.tokenize(Message));
		}
