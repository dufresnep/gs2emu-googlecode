...
{
	// Load File
	std::vector<CString> fileData = CString::loadToken(CString() << "accounts/" << pAccount << ".txt", "\n");
	...
	for (unsigned int i = 0; i < fileData.size(); i++)
	{
		...
		else if (section == "X") x = strtofloat(val);
		else if (section == "Y") y = strtofloat(val);
		else if (section == "MAXHP") maxPower = strtoint(val);
		else if (section == "HP") power = strtofloat(val);
		...
		else if (section == "RATING") rating = strtofloat(val);
		else if (section == "DEVIATION") deviation = strtofloat(val);
		else if (section == "OLDDEVIATION") oldDeviation = strtofloat(val);
...
bool TAccount::saveAccount(bool pOnlyAccount)
{
		...
		else
	{
		...
		newFile.save(CString() << "accounts/" << accountName << ".txt");
	}
